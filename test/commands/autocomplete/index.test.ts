...
skipWindows('_autocomplete index', () => {
  test
  .stdout()
  .command(['_autocomplete', 'bash'])
  .it('provides bash instructions', ctx => {
    expect(ctx.stdout).to.contain(`
Setup Instructions for OCLIF-EXAMPLE CLI Autocomplete ---

1) Add the autocomplete env var to your bash profile and source it
$ printf \"$(oclif-example _autocomplete:script bash)\" >> ~/.bashrc; source ~/.bashrc
...
`,
  ...
  test
  .stdout()
  .command(['_autocomplete', 'zsh'])
  .it('provides zsh instructions', ctx => {
    expect(ctx.stdout).to.contain(`
Setup Instructions for OCLIF-EXAMPLE CLI Autocomplete ---

1) Add the autocomplete env var to your zsh profile and source it
$ printf \"$(oclif-example _autocomplete:script zsh)\" >> ~/.zshrc; source ~/.zshrc
